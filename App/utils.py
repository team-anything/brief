'''
TODO: 
    flag for languages, use same script as employed for INDIC languages.
    get x words only.
    Add stopwords files
'''
import re
...
def summarizer(article):
    iso_lang = detect(article.title)
    if iso_lang in INDIAN_LANGUAGES:
        return summary(article.text, article.title, iso_lang)
    else:
...
def summarize(title="", text="", lang="", max_sents=MAX_SENT):
